...
class VerifyReport:
    ...
    def to_markdown(self) -> str:
        ...
        problem_results = [
            ...
            if result.status in {"mismatch", "not_found", "error"}
        ]
        if problem_results:
            lines.extend(["## Issues", ""])
...
async def _verify_entry(
    ...
    if remote is None:
        return VerificationResult(
            ...
            mismatches=[],
            title_similarity=None,
            notes="No matching paper found on Semantic Scholar or OpenAlex. Possible hallucinated reference.",
        )

    similarity = title_similarity(local_title, remote.title) if local_title else None
    mismatches: list[str] = []
    notes: str | None = None

    if local_title and similarity is not None and similarity < 0.8:
        mismatches.append(
            f"title similarity below threshold: local='{local_title}', remote='{remote.title}'"
        )

    if local_year and remote.year is not None:
        try:
            local_year_int = int(local_year)
        except ValueError:
            mismatches.append(f"year: local={local_year}, remote={remote.year}")
        else:
            if abs(local_year_int - remote.year) > 1:
                mismatches.append(f"year: local={local_year_int}, remote={remote.year}")

    if local_doi and remote.doi and local_doi.lower() != remote.doi.lower():
        mismatches.append(f"doi: local={local_doi}, remote={remote.doi}")

    if local_authors and remote.authors and not _authors_match(local_authors, remote.authors):
        mismatches.append(
            f"authors: local first-author='{_first_author_surname(local_authors)}', "
            f"remote='{', '.join(remote.authors[:3])}'"
        )

    if local_venue and remote.venue and not _venues_match(local_venue, remote.venue):
        mismatches.append(f"venue: local={local_venue}, remote={remote.venue}")

    status = "verified" if not mismatches else "mismatch"
    if status == "mismatch" and any(mismatch.startswith("year:") for mismatch in mismatches):
        notes = (
            f"Year discrepancy: local .bib says {local_year}, "
            f"but remote metadata reports {remote.year}"
        )
    ...
    return VerificationResult(
        ...
        remote_year=remote.year,
        remote_doi=remote.doi,
        remote_authors=remote.authors or None,
        remote_venue=remote.venue,
        remote_citation_count=remote.citation_count,
...
async def _lookup_remote_record(
    ...
    if local_doi:
        ...
        s2_result = _unwrap_remote_response(s2_response)
        openalex_result = _unwrap_remote_response(openalex_response)
...
def _select_entries(
    ...
    *,
    ...
    selected_entries = [entry for entry in selected_entries if entry.fields.get("title") or entry.fields.get("doi")]
    skipped.extend(
        entry.key
        for entry in entries
        if entry not in selected_entries and (selected_keys is None or entry.key in selected_keys)
...
def _unwrap_remote_response(response: PaperResult | None | Exception) -> PaperResult | None:
